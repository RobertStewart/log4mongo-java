 ...
 * 
 * <b>Note:</b> these tests require that a MongoDB server is running, and (by
 * default) assumes that server is listening on the default port (27017) on
 * localhost.
 * <p>
 * Unless a replica set is configured with mongod instances listening on ports
 * 27017 and 27018, errors will be logged. If a host is not listening on port
 * 27018, testOneHostNonDefaultPort() will fail with an error.
 * 
 * To test on localhost with a replica set, do the following (either starting
 * mongod instances in separate terminals or start them with --fork):
 * <ul>
 * <li>$ mkdir -p /data/r0
 * <li>$ mkdir -p /data/r1
 * <li>$ mkdir -p /data/r2
 ...
 * <li>$ mongo
 * <li>>config = {"_id": "foo", members:[{_id: 0, host: 'localhost:27017'},{_id:
 * 1, host: 'localhost:27018'},{_id: 2, host: 'localhost:27019', arbiterOnly:
 * true}]}
 * <li>>rs.initiate(config)
 * <li>Then wait about a minute until replica set is established. You can run
 * rs.status() and look for direct confirmation.
 * </ul>
 * Since the unit tests create and drop databases several times, they run about
 * twice as fast if mongod is started with the --smallfiles argument.
 * 
...
public class TestMongoDbAppenderHosts {
	...
	MongoDbAppender appender = (MongoDbAppender) Logger.getRootLogger()
		.getAppender(MONGODB_APPENDER_NAME);
    ...
    /**
     * If this test is run without a mongod running on localhost port 27018, an
     * error will be logged to the console by the appender.
     * 
     * @throws Exception
     */
    @Test
	...
	props.put("log4j.appender.MongoDB",
