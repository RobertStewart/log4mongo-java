...
public class MongoDbAppender extends BsonAppender {
	...
	public void activateOptions() {
		try {
			...
			errorHandler.error("Unexpected exception while initialising MongoDbAppender.",
	...
	public void setAppendMDCVariables(boolean appendMDCVariables) {
		LoggingEventBsonifier bsonifier = getBsonifier();
		if (bsonifier != null) {
			bsonifier.setAppendMDCVariables(appendMDCVariables);
		}
	}
	...
	private List<ServerAddress> getServerAddresses(String hostname, String port) {
		...
		if (ports.length != 1 && ports.length != hosts.length) {
			...
			if (portNums.size() != 1 && portNums.size() != hosts.length) {
				errorHandler.error(
						"MongoDB appender port property must contain one port or a valid port per host",
						null, ErrorCode.ADDRESS_PARSE_FAILURE);
			} else {
				...
				for (String host : hosts) {
					...
					try {
						...
						errorHandler.error(
								"MongoDB appender hostname property contains unknown host",
	...
	private List<Integer> getPortNums(String[] ports) {
		...
		for (String port : ports) {
			try {
				...
				errorHandler.error(
						"MongoDB appender can't parse a port property value into an integer",
						e, ErrorCode.ADDRESS_PARSE_FAILURE);
			}
