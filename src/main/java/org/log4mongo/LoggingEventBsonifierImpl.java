...
public class LoggingEventBsonifierImpl implements LoggingEventBsonifier {
	private boolean appendMDCVariables = false;
	...
	public void setAppendMDCVariables(boolean appendMDCVariables) {
		this.appendMDCVariables = appendMDCVariables;
	}

	public boolean getAppendMDCVariables() {
		return appendMDCVariables;
	}
	...
	@SuppressWarnings(value = "unchecked")
	private void addMDCVariables(DBObject result) {
		if (appendMDCVariables) {
			final Map<String, Object> mdc = MDC.getContext();
			if (mdc != null) {
				for (Map.Entry<String, Object> entry: mdc.entrySet()) {
					nullSafePut(result, entry.getKey(), entry.getValue());
				}
			}
		}
	}
